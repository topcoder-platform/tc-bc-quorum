...
contract PrivateProject {
    ...
    function createProject(string projectId, string clientId, string copilotId,
        string name, string description, uint budget, string status, string createdBy, string updatedBy) public only2Roles("manager", "client") {
        projects.length++;
        ...
        projects[currentIndex].budget = budget;
    ...
    function updateProject(string projectId, string clientId, string copilotId,
        string name, string description, uint budget, string status, string createdBy, string updatedBy) public onlyRole("manager") {
    ...
    function getProjectsCount() public view only2Roles("manager", "client") returns (uint) {
    ...
    function getProjectPart2(uint index) public view returns (uint, string, string, string) {
        Project memory project = projects[index];
    ...
    function getProjectByIdPart2(string id) public view returns (uint, string, string, string) {
        Project memory project = getProjectFromIdMap(id);
        return flattenProjectPart2(project);
    }
    ...
    function flattenProjectPart2(Project project) private pure returns (uint, string, string, string) {
        return (project.budget, project.status, project.createdBy, project.updatedBy);
    }
